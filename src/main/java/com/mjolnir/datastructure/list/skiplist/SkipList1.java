package com.mjolnir.datastructure.list.skiplist;
 ...
 * @date 2018/12/12
 */
public class SkipList1<T extends Comparable<? super T>> {
    ...
    public SkipList1(int level) {
        if (level < 2){
            level = 2;
        }
        else if (level > 63){
            //up to 63 levels due to the algorithm of choosing level for new node
            level = 63;
        }
        this.level = level;
        this.head = new Node<>(null, level);
    }
    ...
    public static void main(String[] args) {
        SkipList1<Integer> skipList = new SkipList1<Integer>(60);
        skipList.add(1);
        skipList.add(3);
        skipList.add(2);
        skipList.add(4);
